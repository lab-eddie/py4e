    ...
    if check_num is not None:
        if int(id_num[2:4]) > 12 or int(id_num[4:6]) > 31: # 생년월일이 올바른지 체크
            ...
            while a :
                ...
                if millennium == "o":
                    if int(id_num[7]) == 3: s, a = "남자", 0
                    elif int(id_num[7]) == 4: s, a = "여자", 0
                    else : print("입력값이 잘못되었습니다. 다시 입력해주세요")
                    year = f"20{id_num[0:2]}" # 출생년도 변환
                    
                elif millennium =="x":
                    if int(id_num[7]) == 1: s, a = "남자", 0
                    elif int(id_num[7]) == 2: s, a = "여자", 0
                    else : print("입력값이 잘못되었습니다. 다시입력해주세요")
